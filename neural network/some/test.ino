...
// Training data (x1 = previous day's closing price, x2 = volume, x3 = previous week's average price, x4 = market cap, x5 = news sentiment, y_true = next day's closing price increase (1) or decrease (0))
...
float sigmoid(float x) {
    // Sigmoid function: squashes values between 0 and 1, kinda like a probability
    return 1.0 / (1.0 + exp(-x));
}

float sigmoidDerivative(float x) {
    // Derivative of the Sigmoid function: tells us how the sigmoid output changes based on its input
    float s = sigmoid(x);
    return s * (1 - s);
}

float predict(int x1, int x2, int x3, int x4, int x5) {
    // Combine inputs with weights, add bias, and pass through sigmoid function to get a prediction
    return sigmoid((w1 * x1) + (w2 * x2) + (w3 * x3) + (w4 * x4) + (w5 * x5) + b);
}

void train(int epochs) {
    // Training function: runs the training process for a specified number of epochs (iterations)
